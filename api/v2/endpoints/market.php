...
if ($_POST['type'] == 'add_cart') {
    ...
    try {
        ...
        $db->where('hash_id', $wo['hash_id'])->update(T_USER_ORDERS, array(
            'tracking_url' => $wo['tracking_url'],
    ...
    try {
        ...
        $db->insert(T_REFUND, array(
            'order_hash_id' => $wo['hash_id'],
            'user_id' => $wo['user']['user_id'],
    ...
    try {
        ...
        $id = $db->insert(T_PRODUCT_REVIEW, array(
            'user_id' => $wo['user']['user_id'],
        ...
        if (!empty($id)) {
            if (!empty($files)) {
                foreach ($files as $key => $value) {
                    $db->insert(T_ALBUMS_MEDIA, array(
                        'review_id' => $id,
                        'image' => $value
                    ));
                }
