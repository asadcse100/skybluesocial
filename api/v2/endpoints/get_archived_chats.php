<?php
if (!empty($_POST['type']) && in_array($_POST['type'], ['user', 'page', 'group'])) {
    $db->where('type', Wo_Secure($_POST['type']));
}
        ...
        if ($_POST['type'] != 'group') {
            ...
            if (!empty($chat)) {
                ...
                if (!empty($new_data) && !empty($new_data['username'])) {
                    ...
                    if (!empty($new_data['last_message'])) {
                        foreach ($non_allowed as $key5 => $value5) {
            ...
            $group_data = Wo_GetChatGroupData($value->chat_id);
