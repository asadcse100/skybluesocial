...
if (empty($_POST['phone'])) {
	$error_code    = 3;
	$error_message = 'phone (POST) is missing';
}
...
if (empty($error_code)) {
	$recipient_id = Wo_UserIdFromPhoneNumber($_POST['phone']);
	...
	if (empty($recipient_data)) {
		...
		if ($wo['loggedin'] == true && $wo['config']['profileVisit'] == 1 && !empty($_POST['send_notify']) && $_POST['send_notify'] == 1) {
			if ($recipient_data['user_id'] != $wo['user']['user_id'] && $wo['user']['visit_privacy'] == 0) {
				if ($wo['config']['pro'] == 1) {
					if ($recipient_data['is_pro'] == 1 && in_array($recipient_data['pro_type'],
					array_keys($wo['pro_packages'])) && $wo['pro_packages'][$recipient_data['pro_type']]['profile_visitors'] == 1) {
						$can_ = 1;
					}
				} else {
					$can_ = 1;
